...
| `/macro/interest-rates` | Four banks (FED, ECB, BOE, BOJ) against Financial Datasets' ten; `/banks` lists only those. Each rate is read from the bank's own page: the Fed target-range midpoint, the ECB deposit facility rate, the BOE Bank Rate. `date` is the decision date, where Financial Datasets reports the month. `bank` filters; `start_date`/`end_date` answer `bad_request` because no history is sourced. Measured 2026-09-04, the Bank of Japan's page answered "Network Busy" to the scraper, so BOJ was omitted. |
