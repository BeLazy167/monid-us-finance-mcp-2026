...
All 27 Financial Datasets MCP tool names are implemented against live Monid routes and contract tested. The advertised names, descriptions and input schemas are diffed against the captured Financial Datasets surface by test, so the two cannot silently drift.

Call any of them over MCP at `/mcp` or `/api`.

## REST coverage

50 of the 54 Financial Datasets REST paths are registered. 48 return data. Two answer a zero-cost `not_implemented` stub, and four are not registered at all.
...
<Tabs>
  ...
  <Tab title="Not registered, 4 routes">
    The four `as-reported` statement routes: `/financials/as-reported`, `/financials/income-statements/as-reported`, `/financials/balance-sheets/as-reported`, `/financials/cash-flow-statements/as-reported`.

    These return as-filed XBRL with a nested `line_items` tree, which means parsing a filing's rendered statement files rather than reading a normalized feed.
  </Tab>
</Tabs>
...
| `/macro/interest-rates/banks` | Four banks (FED, ECB, BOE, BOJ) against Financial Datasets' ten. We list only banks we actually scrape. |
...
Measured 2026-09-04 against Apple's FY2025 10-K. The normalized statements feed behind `/financials/cash-flow-statements` reports **Net Cash From Investing Activities as 27,910,000,000**. The filing says **15,195,000,000**. The gap is exactly Apple's property-plant-and-equipment purchase of 12,715,000,000, which that feed omits from the investing subtotal while still using it to compute Free Cash Flow correctly.

The error propagates to `change_in_cash_and_equivalents`. Operating and financing activities match the filing exactly.

This is an upstream aggregation defect, not a parsing bug on our side, and it will be wrong for any company with material capital expenditure. Treat the investing and net-change lines from this route as unreliable until it is re-sourced. Operating cash flow, free cash flow and the income statement are unaffected.
...
<AccordionGroup>
  ...
  <Accordion title="Earnings composition is narrower">
    `get_earnings` composes records from 10-K and 10-Q filing events only. 8-K earnings releases and the market-wide, no-ticker feed are not routed. Records omit any block whose period is absent from the underlying statements matrix.
  </Accordion>
...
Do not migrate yet if you depend on `as_reported` statements, `cik`-keyed company lookup, market-wide news, an unscoped IPO or investor listing, or the cash-flow investing line described above. File an issue on [GitHub](https://github.com/BeLazy167/monid-us-finance-mcp-2026) if you need one of those.
