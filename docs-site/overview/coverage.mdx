...
| `/macro/interest-rates` | Three banks (FED, ECB, BOE) against Financial Datasets' ten; `/banks` lists only those. Each rate is read from the bank's own page: the Fed target-range midpoint, the ECB deposit facility rate, the BOE Bank Rate. `date` is the decision date, where Financial Datasets reports the month. `bank` filters; `start_date`/`end_date` answer `bad_request` because no history is sourced. The Bank of Japan is not covered: it publishes decisions only as PDF statements, which the markdown scraper cannot read. |
